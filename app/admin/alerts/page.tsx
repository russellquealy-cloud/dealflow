...
export default function AdminAlerts() {
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto' }}>
      ...
      <h1 style={{ marginBottom: '30px' }}>Alerts Management (Admin)</h1>
      ...
      <div style={{
        ...
        <ul style={{ margin: '10px 0 0 0', paddingLeft: '20px', color: '#856404' }}>
          <li>Real-time alert creation and management</li>
          <li>Email/SMS notifications</li>
          <li>Alert triggers based on property criteria</li>
      ...
      {loading ? (
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            <h2>User Alerts ({alerts.length})</h2>
            <button style={{
              padding: '10px 20px',
              background: '#007bff',
              color: 'white',
          ...
          {alerts.length === 0 ? (
            <div style={{
              ...
              <p style={{ margin: '0', color: '#6c757d' }}>No alerts found</p>
            </div>
          ) : (
              ...
              {alerts.map((alert: Record<string, unknown>) => (
                <div key={String(alert.id)} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                    <div>
                      <h4 style={{ margin: '0 0 10px 0' }}>{String(alert.name) || 'Untitled Alert'}</h4>
                      ...
                      <div style={{ display: 'flex', gap: '20px', fontSize: '14px', color: '#6c757d' }}>
                        <span>Type: {String(alert.alert_type) || 'Property'}</span>
                        <span>Status: {alert.is_active ? 'Active' : 'Inactive'}</span>
                        <span>Created: {new Date(String(alert.created_at)).toLocaleDateString()}</span>
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '10px' }}>
                      <button style={{
                        padding: '5px 10px',
                        background: alert.is_active ? '#dc3545' : '#28a745',
                        color: 'white',
                        border: 'none',
                        borderRadius: '4px',
                        cursor: 'pointer',
