...
const money = (n?: number) =>
  typeof n === 'number'
    ...
    : '—';
const pick = (obj: any, keys: string[]) => { // safe narrow on read-only surface props
  for (const k of keys) if (obj?.[k] !== undefined && obj?.[k] !== null && obj?.[k] !== '') return obj[k];
  return undefined;
};

export default function ListingCard({ listing }: { listing: Record<string, unknown> }) {
  ...
  const spread = typeof arv === 'number' && typeof price === 'number'
    ? arv - (price + (repairs ?? 0)) : undefined;
  const roi = typeof arv === 'number' && typeof price === 'number'
    ? ((arv - (price + (repairs ?? 0))) / (price + (repairs ?? 0))) * 100 : undefined;

  const hero = pick(listing, ['hero_url', 'photo_url', 'image_url', 'coverImage']) as string | undefined;
  ...
  const addrLine = [street, city, state, zip].filter(Boolean).join(', ');

  return (
    <div className="rounded-xl border bg-white overflow-hidden">
      {hero && <img src={hero} alt={String(addrLine || 'Listing photo')} className="h-40 w-full object-cover" />}
