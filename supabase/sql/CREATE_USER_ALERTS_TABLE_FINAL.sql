...
-- FINAL VERSION: Assumes 'role' column exists (run ADD_ROLE_COLUMN_TO_PROFILES.sql first)
...
-- This version uses the 'role' column directly
DO $$
DECLARE
  ...
  user_role_val TEXT;
BEGIN
  -- Use segment column directly (it exists in your table!)
  FOR user_rec IN 
    SELECT p.id, p.segment as role_value
    FROM profiles p
    WHERE p.segment IN ('investor', 'wholesaler')
  LOOP
    user_role_val := user_rec.role_value;
